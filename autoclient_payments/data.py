...
from autoclient_payments.messages import (
    ...
    DESC_FUNDS_ALL,
    DESC_COMPLAINT_STATUS_NOT_PENDING,
)
...
def payment_message_list_status(messages):
    return payment_message_status(payment_primary_message(messages))
...
def processing_message_list_description(messages):
    return processing_message_description(payment_message_list_status(messages))


def processing_message_failed_list_description(messages):
    return processing_message_failed_description(payment_primary_message(messages))
...
def lazy_set_author(func):
    def wrapper(data):
        if not data.get("author"):
            from autoclient_payments.cached import get_complaint
            ...
            if complaint_data:
                ...
                if author and complaint_data.get("status") != STATUS_COMPLAINT_DRAFT:
                    from autoclient_payments.results_db import set_payment_complaint_author
...
def value_amount_representation(value):
    return str(amount_convert(value))
...
PAYMENTS_NOT_FAILED_MESSAGE_ID_LIST = (
    PAYMENTS_INFO_MESSAGE_ID_LIST + PAYMENTS_SUCCESS_MESSAGE_ID_LIST + PAYMENTS_WARNING_MESSAGE_ID_LIST
)
...
DEFAULT_MESSAGE_STATUS = "default"
...
DESC_PROCESSING_CHOICES_DICT = {
    DEFAULT_MESSAGE_STATUS: DESC_PROCESSING_DEFAULT,
    NEUTRAL_MESSAGE_STATUS: DESC_PROCESSING_NEUTRAL,
    WARNING_MESSAGE_STATUS: DESC_PROCESSING_WARNING,
    DANGER_MESSAGE_STATUS: DESC_PROCESSING_DANGER,
    SUCCESS_MESSAGE_STATUS: DESC_PROCESSING_SUCCESS,
