...
class GetItemDataTestCase(unittest.TestCase):
    ...
    def test_item_found(self):
        ...
        self.assertEqual(get_item_data(data, "items", second_item["id"]), second_item)

    def test_item_not_found(self):
        first_item = {"id": "1", "value": "fail"}
        second_item = {"id": "2", "value": "success"}
        data = {"items": [first_item, second_item]}
        self.assertEqual(get_item_data(data, "items", "invalid_id"), None)
...
class GetComplaintSearchUrlTestCase(unittest.TestCase):
    ...
    def test_get_complaint_search_url(self):
        ...
        self.assertEqual(
            result, url_pattern.format(host=API_HOST, version=API_VERSION, complaint_pretty_id=complaint_pretty_id)
        )
...
class RequestComplaintSearchTestCase(unittest.TestCase):
    ...
    @patch("autoclient_payments.utils.uuid4", MagicMock())
    @patch("autoclient_payments.utils.requests")
    def test_request_complaint_search(self, requests):
        ...
        requests.get.assert_called_once_with(url, headers=headers, timeout=timeout, cookies=None)
    ...
    def test_request_complaint_search_with_optional_args(self, requests):
        ...
        requests.get.assert_called_once_with(url, headers=headers, timeout=timeout, cookies=cookies)
...
class RequestComplaintDataTestCase(unittest.TestCase):
    ...
    def test_request_complaint_data(self, requests):
        ...
        result = request_cdb_complaint_data(tender_id, item_type, item_id, complaint_id)
...
class RequestComplaintPatchTestCase(unittest.TestCase):
    ...
    def test_request_complaint_patch(self, requests):
        ...
        result = request_cdb_complaint_patch(tender_id, item_type, item_id, complaint_id, data)

        self.assertEqual(result, requests.patch.return_value)

        requests.patch.assert_called_once_with(url, json=json, headers=headers, timeout=timeout, cookies=None)
    ...
    def test_request_complaint_patch_with_optional_args(self, requests):
        ...
        requests.patch.assert_called_once_with(url, json=json, headers=headers, timeout=timeout, cookies=cookies)


class GetPaymentsRegistryFakeTestCase(unittest.TestCase):
    @patch("autoclient_payments.utils.shelve")
    def test_get_payments_registry_fake(self, shelve):
        date_from = datetime.now() - timedelta(days=1)
        date_to = datetime.now() + timedelta(days=1)
        shelve.open.return_value.__enter__.return_value = {
            "registry": [{"DATE_TIME_DAT_OD_TIM_P": datetime.now().strftime("%d.%m.%Y %H:%M:%S")}]
        }

        result = get_payments_registry_fake(date_from, date_to)

        shelve.open.assert_called_once_with("autoclient_payments.db")
        self.assertEqual(result, shelve.open.return_value.__enter__.return_value["registry"])

    @patch("autoclient_payments.utils.shelve")
    def test_get_payments_registry_fake_invalid_date_oper_format(self, shelve):
        date_from = datetime.now() - timedelta(days=1)
        date_to = datetime.now() + timedelta(days=1)
        shelve.open.return_value.__enter__.return_value = {"registry": [{"DATE_TIME_DAT_OD_TIM_P": "test"}]}
...
class DumpsPaymentsRegistryFakeTestCase(unittest.TestCase):
    @patch("autoclient_payments.utils.json")
    @patch("autoclient_payments.utils.shelve")
    def test_dumps_payments_registry_fake(self, shelve, json):
        ...
        json.dumps.assert_called_once_with(
            shelve.open.return_value.__enter__.return_value.get.return_value, indent=4, ensure_ascii=False
        )
...
class StorePaymentsRegistryFakeTestCase(unittest.TestCase):
    ...
    def test_store_payments_registry_fake(self, shelve, json):
        ...
        shelve.open.assert_called_once_with("autoclient_payments.db")
        self.assertEqual(shelve.open.return_value.__enter__.return_value["registry"], json.loads.return_value)
    ...
    def test_store_payments_registry_fake_json_decode_error(self, shelve, json):
        ...
        self.assertEqual(shelve.open.return_value.__enter__.return_value, dict())
    ...
    def test_store_payments_registry_fake_none(self, shelve, json):
        ...
        shelve.open.assert_called_once_with("autoclient_payments.db")
        self.assertEqual(shelve.open.return_value.__enter__.return_value["registry"], None)
...
class PutPaymentsRegistryFakeDataTestCase(unittest.TestCase):
    ...
    def test_put_payments_registry_fake_data(self, shelve):
        ...
        shelve.open.assert_called_once_with("autoclient_payments.db")
        self.assertEqual(shelve.open.return_value.__enter__.return_value["registry"], data)
    ...
    def test_put_payments_registry_fake_data_os_error(self, shelve):
        ...
        shelve.open.assert_called_once_with("autoclient_payments.db")
        self.assertEqual(shelve.open.return_value.__enter__.return_value, None)
...
class GetTransactions(unittest.TestCase):
    @patch("autoclient_payments.utils.requests.get")
    def test_get_transactions(self, mock_get):
        ...
        self.assertEqual(next_page_id, "abc123")
        mock_response.raise_for_status.assert_called_once()

    @patch("autoclient_payments.utils.get_transactions")
    def test_transactions_list(self, mock_get_transactions):
        ...
        result = list(transactions_list(start_date="01-01-2026"))
