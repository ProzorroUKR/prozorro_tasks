...
from autoclient_payments.tasks import process_complaint_resolution
from autoclient_payments.message_ids import PAYMENTS_CRAWLER_RESOLUTION_SAVE_SUCCESS


class TestHandlerCase(unittest.TestCase):
    def test_handle_mongodb_error(self):
        payment_data = {"test_param": "test_value"}
        complaint_data = {"id": uuid4().hex, "status": "mistaken"}
        ...
        with patch("autoclient_payments.results_db.get_mongodb_collection") as get_collection:
            ...
            with self.assertRaises(Retry):
                process_complaint_resolution(payment_data=payment_data, complaint_data=complaint_data)
    ...
    def test_handle_resolution_mistaken(self):
        ...
        for reject_reason in ["incorrectPayment", "complaintPeriodEnded", "cancelledByComplainant"]:
            ...
            with patch("autoclient_payments.tasks.set_payment_resolution") as set_payment_resolution, \
                 patch("autoclient_payments.logging.push_payment_message") as push_payment_message:
                process_complaint_resolution(payment_data=payment_data, complaint_data=complaint_data)

                set_payment_resolution.assert_called_once_with(payment_data, resolution)
