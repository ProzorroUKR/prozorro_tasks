...
from autoclient_payments.tasks import process_complaint_params
...
class TestHandlerCase(unittest.TestCase):
    def test_handle_mongodb_error(self):
        ...
        with patch("autoclient_payments.results_db.get_mongodb_collection") as get_collection:
            collection = Mock()
            get_collection.return_value = collection
            collection.find_one.side_effect = pymongo.errors.PyMongoError()

            with self.assertRaises(Retry):
                process_complaint_params(complaint_params=complaint_params, complaint_data=complaint_data)

        process_complaint_params.retry.assert_called_once_with(
            countdown=DEFAULT_RETRY_AFTER, exc=collection.find_one.side_effect
        )

    @patch("autoclient_payments.tasks.process_complaint_resolution")
    def test_handle_payment(self, process_complaint_resolution):
        ...
        with patch("autoclient_payments.tasks.get_payment_item_by_params") as get_payment_item_by_params:
    ...
    def test_handle_payment_resolution_already_exists(self, process_complaint_resolution):
        ...
        payment = {"payment": {"description": "Test description"}, "resolution": {"test_res_field": "test_res_value"}}
