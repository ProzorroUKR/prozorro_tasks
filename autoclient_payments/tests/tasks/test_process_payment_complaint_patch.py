...
from autoclient_payments.tasks import process_payment_complaint_patch
...
class TestHandlerCase(unittest.TestCase):
    ...
    def test_handle_429_response(self):
        ...
        process_payment_complaint_patch.retry.assert_called_once_with(countdown=ret_aft)
    ...
    def test_handle_412_response(self):
        ...
        with patch("autoclient_payments.utils.requests") as requests_mock, \
             patch("autoclient_payments.logging.push_payment_message") as push_payment_message:
            requests_mock.patch.return_value = Mock(status_code=412, cookies=Mock(get_dict=Mock(return_value=cookies)))
    ...
    def test_handle_500_response(self):
        ...
        process_payment_complaint_patch.retry.assert_called_once_with(countdown=DEFAULT_RETRY_AFTER)
    ...
    @patch("autoclient_payments.tasks.process_payment_complaint_recheck")
    def test_handle_403_response(self, process_payment_complaint_recheck):
        ...
        with patch("autoclient_payments.utils.requests") as requests_mock, \
             patch("autoclient_payments.logging.push_payment_message") as push_payment_message:
            requests_mock.patch.return_value = Mock(
                status_code=403, cookies=Mock(get_dict=Mock(return_value=cookies)), headers={}
            )
    ...
    def test_handle_200_response_complaint_pending(self):
        ...
        with patch("autoclient_payments.utils.requests") as requests_mock, \
             patch("autoclient_payments.logging.push_payment_message") as push_payment_message:
            requests_mock.patch.return_value = Mock(status_code=200, cookies=Mock(get_dict=Mock(return_value=cookies)))
