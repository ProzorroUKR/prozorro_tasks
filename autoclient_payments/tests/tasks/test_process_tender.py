...
from autoclient_payments.data import STATUS_COMPLAINT_DRAFT, STATUS_COMPLAINT_PENDING, STATUS_COMPLAINT_ACCEPTED
...
class TestHandlerCase(unittest.TestCase):
    ...
    def test_handle_429_response(self):
        ...
        ret_aft = 13
        with patch("autoclient_payments.utils.requests") as requests_mock:
            requests_mock.get.return_value = Mock(status_code=429, headers={"Retry-After": ret_aft})
    ...
    def test_handle_500_response(self):
        ...
        with patch("autoclient_payments.utils.requests") as requests_mock:
            requests_mock.get.return_value = Mock(status_code=500, headers={})
    ...
    def test_handle_404_response(self):
        ...
        with patch("autoclient_payments.utils.requests") as requests_mock:
            requests_mock.get.return_value = Mock(status_code=404, headers={})
    ...
    @patch("autoclient_payments.tasks.process_complaint_params")
    def test_handle_200_response_tender_complaints(self, process_complaint_params):
        tender_id = uuid4().hex

        allowed_complaints_data = [
            {"id": uuid4().hex, "status": status} for status in ALLOWED_COMPLAINT_RESOLUTION_STATUSES
        ]
        ...
        with patch("autoclient_payments.utils.requests") as requests_mock:
            requests_mock.get.return_value = Mock(
                ...
                json=Mock(
                    return_value={
                        "data": {"id": tender_id, "complaints": allowed_complaints_data + not_allowed_complaints_data},
                    }
                ),
            )
                ...
                json=Mock(
                    return_value={
                        "data": {
                            "id": tender_id,
                            "awards": [
                                {"id": award_id, "complaints": allowed_complaints_data + not_allowed_complaints_data}
                            ],
                        },
                    }
                ),
