...
@app.task(bind=True, max_retries=CHRONOGRAPH_CHECK_MAX_RETRIES)
def recheck_agreement(self, agreement_id, cookies=None):
    url = "{host}/api/{version}/agreements/{agreement_id}".format(
        host=API_HOST,
        version=API_VERSION,
    ...
    try:
        ...
        if response.status_code != 200:
            logger_method = get_task_retry_logger_method(self, logger)
            logger_method("Unexpected status code {} while getting framework {}".format(
                response.status_code, agreement_id
            ), extra={
                "MESSAGE_ID": "CHRONOGRAPH_AGREEMENT_GET_CODE_ERROR",
                "STATUS_CODE": response.status_code,
            })
            if response.status_code == 412:  # Precondition failed
                retry_kwargs = dict(**self.request.kwargs)
    ...
    if "next_check" not in response.json()["data"]:
        logger.warning("Skip framework {} without next_check".format(
            ...
            "MESSAGE_ID": "CHRONOGRAPH_AGREEMENT_SKIP",
        })
        return
    ...
    try:
        ...
        logger.exception(exc, extra={"MESSAGE_ID": "CHRONOGRAPH_AGREEMENT_CHECK_EXCEPTION"})
        raise self.retry(exc=exc)
    else:
        if response.status_code != 200:
            logger_method = get_task_retry_logger_method(self, logger)
            logger_method("Unexpected status code {} while checking agreement {}".format(
                response.status_code, agreement_id
            ), extra={
                "MESSAGE_ID": "CHRONOGRAPH_AGREEMENT_CHECK_CODE_ERROR",
                "STATUS_CODE": response.status_code,
            })
            if response.status_code == 412:  # Precondition failed
                retry_kwargs = dict(**self.request.kwargs)
